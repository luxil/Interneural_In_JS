...
function makeNetworkGraph() {
    ...
    function initForce() {
        force = d3.layout.force()
            ...
            .linkDistance(function (l, i) {
                return l.source.bias ? linkDistance / 10 : linkDistance
            })
            .size([width, height]);
    ...
    function restart() {
        ...
        node.enter().append("circle")
            .attr("class", function (d) {
                return d.fCenter ? "force_center" : ""
            })
            .attr("r", function (d) {
                return 20 - (d.bias ? 10 : 0);
            })
            .style("fill", function (d) {
                return d.color ? d.color : layerColors[d.layer];
            })
            .style("stroke", "white")
    ...
    function createNode(nodeId, layerNo, charge, bias, lastLayer, iInLayer) {
        ...
        if (lastLayer) {
            node.color = colorCodes[iInLayer];
        }
        nodes.push(node);
        return node;
    }
    ...
    function updateLink(sourceId, targetId, weight) {
        ...
        var link = $.grep(links, function (l) {
            return (l.source.id == sourceId) && (l.target.id == targetId);
        })[0];
        link.weight = weight;
        return link;
    }
